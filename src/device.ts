...
export class UlanziD200 extends EventEmitter {
  ...
  // ── Image cache / layout API ───────────────────────────────
  // Preload images once, then swap layouts with manifest-only ZIPs.
  // This is dramatically faster than re-sending image data each time.

  /** Map of preloaded image names → icon paths used in the device cache */
  private preloadedNames = new Set<string>();

  /**
   * Upload named images to the device's cache. Call this once at startup.
   * Images are resized to 196x196 and stored under `icons/{name}` in the
   * device's cache. Use `setLayout()` to map them to buttons.
   *
   * @param images - Map of name → image Buffer (PNG/JPEG/etc)
   *
   * @example
   * await deck.preloadImages({
   *   'play': fs.readFileSync('play.png'),
   *   'stop': fs.readFileSync('stop.png'),
   *   'frame_0': gifFrames[0],
   *   'frame_1': gifFrames[1],
   * });
   */
  async preloadImages(
    ...
    for (const [name, data] of Object.entries(images)) {
      prepared.set(name, await this.prepareImage(data));
  ...
  /**
   * Set the button layout using preloaded image names. Sends a tiny
   * manifest-only ZIP (no image data) — typically 1 packet, instant.
   *
   * @param layout - Map of button index (0-13) → preloaded image name.
   *   Can also include `label` per button.
   *
   * @example
   * await deck.setLayout({
   *   0: 'play',
   *   1: 'stop',
   *   2: { icon: 'rec', label: 'Record' },
   * });
   */
  async setLayout(
    layout: Record<number, string | { icon: string; label?: string }>,
  ): Promise<void> {
    for (const [indexStr, value] of Object.entries(layout)) {
      const index = Number(indexStr);
      if (index < 0 || index > 13) continue;

      const config: ButtonConfig = {};
      if (typeof value === 'string') {
        config.iconRef = `icons/${value}`;
      } else {
        config.iconRef = `icons/${value.icon}`;
        if (value.label) config.label = value.label;
      }
  ...
  /**
   * Animate a button by cycling through preloaded image names.
   * Each frame is a manifest-only ZIP (~1 packet) so animation is very fast.
   *
   * @param index - Button index (0-13)
   * @param frameNames - Array of preloaded image names to cycle through
   * @param fps - Target frames per second
   *
   * @example
   * await deck.preloadImages({
   *   'cat_0': frames[0], 'cat_1': frames[1], 'cat_2': frames[2],
   * });
   * deck.animateCached(0, ['cat_0', 'cat_1', 'cat_2'], 15);
   */
  animateCached(
    index: number,
    frameNames: string[],
    ...
    if (frameNames.length === 0) throw new Error('No frame names provided');
    ...
    const loop = async () => {
      while (running) {
        ...
        try {
          this.buttonState.set(index, { iconRef: `icons/${frameNames[frameIndex]}` });
